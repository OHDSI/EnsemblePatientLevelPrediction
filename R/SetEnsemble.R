...
#' }
#' @export
setEnsembleFromDesign <- function(
  modelDesignList,
  databaseDetails,
  splitSettings = PatientLevelPrediction::createDefaultSplitSetting(),
  filterSettings,
...
#' }
#' @export
setEnsembleFromResults <- function(
  resultList,
  filterSettings,
  combinerSettings
){
  ...
  indexPpl <- lapply(resultList, function(x) x$prediction %>% dplyr::select(.data$subjectId, .data$cohortStartDate))
  
  # if the models are trained on the same target and index dates we can evaluate
  # otherwise we stop at creating the ensemble
  evaluateEnsemble <- T
  for(i in 1:(length(indexPpl)-1)){
    evaluateEnsemble <- evaluateEnsemble && nrow(merge(indexPpl[[i]], indexPpl[[i+1]], by = c('subjectId', 'cohortStartDate'))) == nrow(indexPpl[[i]])
  }
...
#' Create setting for creating ensemble from model settings
#' @param fileVector   A vector of files containing the location of the \code{PatientLevelPrediction::runPlp()} results
#' @param filterSettings   Setting specifying rules to use to filter (remove) any model specified in the list of model designs that performs insufficiently (these models get ignored from the ensemble)
#' @param combinerSettings  Settings specifying how to combine the remaining models into an ensemble
#'
#' @examples
