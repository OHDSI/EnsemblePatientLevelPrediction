#' Code to run the ensemble model developement
...
runEnsemble <-function(
  ...
  if(ensembleSettings$executionList$trainModels){
    ...
    ensembleSettings$resultList <- lapply(fileList, function(x) PatientLevelPrediction::loadPlpResult(file.path(x,'plpResult')))
  }
  ...
  if(ensembleSettings$executionList$evaluateEnsemble){
    ...
    prediction <- do.call(
      ...
      list(
        predictionList = lapply(ensembleSettings$resultList, function(x) x$prediction),
        ensemble = ensemble
      )
    )
