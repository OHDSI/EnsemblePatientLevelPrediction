createEnsemble <- function(
  ...
  ensembleModel <- do.call(fun, 
                           list(
                             ...
                             baseModelResults = baseModelResults
                           )
                           )
  ...
  result <- list(
    model = list(
      ...
      baseModels = lapply(baseModelResults, function(x){x$model})
    ),
    trainDetails = list(analysisId = 'ensemble'),
    settings = combinerSettings,
    covariateImportance = data.frame() # make this the weights?
  )
